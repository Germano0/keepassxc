...
QJsonObject BrowserAction::handleAction(const QJsonObject& message, QLocalSocket* socket)
{
    ...
    if (browserRequest.action.compare(BROWSER_REQUEST_GET_DATABASE_STATUSES) == 0) {
        ...
        return handlePasskeysGet(browserRequest);
    } else if (action.compare(BROWSER_REQUEST_PASSKEYS_REGISTER) == 0) {
        return handlePasskeysRegister(browserRequest);
#endif
    }
...
QJsonObject BrowserAction::handleAssociate(const BrowserRequest& browserRequest)
{
    ...
    if (publicKey.compare(m_clientPublicKey) == 0) {
        ...
        const ResponseParameters params{{"hash", browserRequest.hash}, {"id", id}};
...
QJsonObject BrowserAction::handleCreateNewGroup(const BrowserRequest& browserRequest)
{
    ...
    const ResponseParameters params{{"name", newGroup["name"]}, {"uuid", newGroup["uuid"]}};
    return buildResponse(browserRequest, params);
}
...
QJsonObject BrowserAction::handleGetCredentials(const BrowserRequest& browserRequest)
{
    ...
    const ResponseParameters params{{"entries", entries}, {"hash", browserRequest.hash}};
    return buildResponse(browserRequest, params);
}
...
QJsonObject BrowserAction::handleGetDatabaseEntries(const BrowserRequest& browserRequest)
{
    ...
    const ResponseParameters params{{"entries", entries}};
    return buildResponse(browserRequest, params);
}
...
QJsonObject BrowserAction::handleGetDatabaseGroups(const BrowserRequest& browserRequest)
{
    ...
    const ResponseParameters params{{"groups", groups}};
    return buildResponse(browserRequest, params);
}
...
QJsonObject BrowserAction::handleGetDatabaseStatuses(const BrowserRequest& browserRequest)
{
    ...
    const ResponseParameters params{{"hash", browserRequest.hash}, {"statuses", databaseStatuses}};
    return buildResponse(browserRequest, params);
}
...
QJsonObject BrowserAction::handleGetTotp(const BrowserRequest& browserRequest)
{
    ...
    const ResponseParameters params{
        {"totpList", browserService()->getTotp(getConnectionKeys(browserRequest), uuidList)}};
    return buildResponse(browserRequest, params);
}
...
QJsonObject BrowserAction::handleGlobalAutoType(const BrowserRequest& browserRequest)
{
    ...
    const ResponseParameters params{{"result", true}};
    return buildResponse(browserRequest, params);
}
...
#ifdef WITH_XC_BROWSER_PASSKEYS
QJsonObject BrowserAction::handlePasskeysGet(const BrowserRequest& browserRequest)
{
    const auto publicKey = browserRequest.getObject("publicKey");
    if (publicKey.isEmpty()) {
        return buildErrorResponse(browserRequest, ERROR_PASSKEYS_EMPTY_PUBLIC_KEY);
    }
    ...
    if (!passkeyUtils()->isOriginAllowedWithLocalhost(browserSettings()->allowLocalhostWithPasskeys(), origin)) {
        return buildErrorResponse(browserRequest, ERROR_PASSKEYS_INVALID_URL_PROVIDED);
    }

    const auto keyList = getConnectionKeys(browserRequest);
    const auto response = browserService()->showPasskeysAuthenticationPrompt(publicKey, origin, keyList);

    const ResponseParameters params{{"response", response}};
    return buildResponse(browserRequest, params);
}

QJsonObject BrowserAction::handlePasskeysRegister(const BrowserRequest& browserRequest)
{
    const auto publicKey = browserRequest.getObject("publicKey");
    if (publicKey.isEmpty()) {
        return buildErrorResponse(browserRequest, ERROR_PASSKEYS_EMPTY_PUBLIC_KEY);
...
{
    const ResponseParameters params{{"errorCode", errorCode},
                                    {"error", browserMessageBuilder()->getErrorMessage(errorCode)}};
    return buildResponse(browserRequest, params);
}

QJsonObject BrowserAction::buildResponse(const BrowserRequest& browserRequest, const ResponseParameters& params) const
{
